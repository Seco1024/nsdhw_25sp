...
1. **Base Index Class**
   ...
   - ``indexBase``: Defines the common API for all indexing methods (e.g.,
     ``add()``, ``search()``, ``train()``, ``write_index()``)

2. **KD-tree Index Class**

   - ``KDTreeIndex``: Serve as a baseline which perform exact search
...
2. Build an index with ``add()`` :
   ...
   - Optimize the index data layout to improve cache locality.

3. Perform a query on the specified index instance using ``search()``.
4. Return result set with top-k id & estimated distance for each query.
